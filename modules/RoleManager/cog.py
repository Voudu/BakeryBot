...
class RoleManager(commands.Cog, name="Role Manager Cog"):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self._messageIds = []   # list of message ids to listen on
        self._msg_role_dict = {}    # nested dictionary, maps message ids to a dict of reactions and roles which it listens for
                                    #
                                    # looks like this
    ...
    async def roleinit(self, ctx: commands.Context, hex, *title):
        ...
        if not self._msg_role_dict:         # dictionary is empty, make sure theres no data to load first
            self.loadObj(ctx)
        ...
        if msgId is not None:                                           # check if the embed message id is valid
            self._msg_role_dict[str(msgId)] = {}                             # a nested dict
    ...
    async def roleadd(self, ctx: commands.Context, msg_id, at_role, react, *identifier_msg):
        ...
        if msgId not in self._msg_role_dict:                               # update the message role dictionary
            ...
            if react in self._msg_role_dict[msgId]:
                ...
                try:
                    ...
                    # await msg.edit(embed=new_embed)                         # send back the edit for the description                        
                    self._msg_role_dict[msgIdKey][emojiKey] = role_id       # add the react and role id to the dictionary indexed at the message id
    ...
    def writeObj(self, ctx: commands.Context):
        ...
        try:
            with open(f'modules/RoleManager/role_data/{guild_id}.json', 'w') as f:
                f.write(json.dumps(self._msg_role_dict))
        except FileNotFoundError as fe:
            print(fe)
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        if not self._msg_role_dict:         # dictionary is empty
            try:                            # try to load the dictionary from the json file
                with open(f'modules/RoleManager/role_data/{guild_id}.json') as f:
                    self._msg_role_dict = json.load(f)
                    print(self._msg_role_dict)
                    print(f"_msg_role_dict loaded as: {type(self._msg_role_dict)}")
            except FileNotFoundError as fe:
                print(fe)
        
        msgId = str(payload.message_id)
        
        if msgId in self._msg_role_dict:                    # check if the message is in the dict
            ...
            roleId = self._msg_role_dict[msgId][reaction]
